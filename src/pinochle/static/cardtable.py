...
from browser import ajax, document, html, svg, window
from brySVG.dragcanvas import UseObject

from constants import CARD_URL, DEBUG, GAME_MODES, OTHER_DECK_CONFIG, PLAYER_DECK_CONFIG
...
class PlayingCard(UseObject):
    ...
    def play_handler(self, event=None):
        ...
        if new_y < card_height or "click" in event.type:
            ...
            if GAME_MODES[GAME_MODE] in ["trick"]:  # Trick
    ...
    def card_click_handler(self, event=None):
        ...
        :type event: [type], optional
        """
        global players_hand, players_meld_deck
        if DEBUG:
...
def on_complete_games(req):
    ...
    for item in temp:
        if DEBUG > 1:
            print(f"on_complete_games: {item=}")
...
def on_complete_rounds(req):
    global ROUND_ID, round_list, team_list
    round_list = []
    team_list = []
    if DEBUG:
        print("In on_complete_rounds.")
    ...
    if DEBUG > 1:
        print(f"on_complete_rounds: {round_list=}")

    display_game_options()


def on_complete_teams(req):
    global TEAM_ID, team_list, team_dict, team_names
    team_dict = {}
    team_names = {}
    if DEBUG:
        print("In on_complete_teams.")
    ...
    if DEBUG > 1:
        print(f"on_complete_teams: {team_list=}")
...
def on_complete_team_names(req):
    global TEAM_ID, team_dict, team_names
    if DEBUG:
        print("In on_complete_team_names.")

    temp = on_complete_common(req)
    if temp is None:
        return

    # Set the global dict of teams for this round.
    team_dict[temp["team_id"]] = temp
    if DEBUG > 1:
        print(f"on_complete_team_names: {team_dict=}")
    display_game_options()
...
def on_complete_players(req):
    ...
    if DEBUG > 1:
        print(f"In on_complete_players: {player_dict=}")
    display_game_options()
...
def on_complete_kitty(req):
    ...
    if DEBUG > 1:
        print(f"on_complete_kitty: {kitty_deck=}")


def on_complete_player_cards(req):
    global players_hand, players_meld_deck
    if DEBUG:
        print("In on_complete_player_cards.")
    ...
    if DEBUG > 1:
        print(f"on_complete_player_cards: {players_hand=}")
...
def on_complete_common(req):
    if DEBUG:
        print("In on_complete_common.")
    if req.status == 200 or req.status == 0:
        temp = json.loads(req.text)
        return temp
    else:
        if DEBUG > 1:
            print(f"on_complete_common: {req=}")
...
def populate_canvas(deck, target_canvas, deck_type="player"):
    ...
    if DEBUG:
        print(
            "Entering populate_canvas(deck={}"
            "target_canvas={} deck_type={}).".format(deck, target_canvas, deck_type)
        )
...
def update_display(event=None):
    ...
    if DEBUG:
        print("Entering update_display. (mode={})".format(mode))
    calculate_dimensions()
    # Place the desired decks on the display.
    # TODO: If there is no kitty in this game, don't bother showing the (empty) kitty
    # deck or allowing the bid winner do anything with them. Unfortunately it doesn't
    # appear to be as easy as incrementing GAME_MODE.
...
def clear_display(event=None):
    ...
    mode = GAME_MODES[GAME_MODE]
    if DEBUG:
        print("Entering clear_display (mode={})".format(mode))
    try:
...
def display_game_options():
    global canvas, game_dict, player_list, player_dict, team_list, team_dict
    ...
    if GAME_ID == "":
        if DEBUG > 1:
            print(f"display_game_options: {game_dict=}")
        for item in game_dict:
            if DEBUG > 1:
                print(f"display_game_options: game_dict item: {item=}")
        ...
        for item in player_dict:
            ...
            if DEBUG > 1:
                print(f"display_game_options: player_list item: {item=}")
            canvas <= round_button
            ypos += 40
...
def choose_player(event=None):
    ...
    if DEBUG > 1:
        print(f"choose_player: {PLAYER_ID=}")
